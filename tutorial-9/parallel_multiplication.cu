...
__global__ void mult(double *d_arr1, double *d_arr2, double *d_prod, int n){
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    if (tid < n){
        d_prod[tid] = d_arr1[tid] * d_arr2[tid];
    }
}
...
int main(){
    ...
    clock_t start = clock();
    mult<<<blocksPerGrid, threadsPerBlock>>>(d_arr1, d_arr2, d_prod, count);
    cudaDeviceSynchronize();
    clock_t end = clock();

    cudaMemcpy(sum, d_prod, count * sizeof(double), cudaMemcpyDeviceToHost); 
